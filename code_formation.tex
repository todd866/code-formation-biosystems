...
The rest of the paper is organised as follows. Section~\ref{sec:problem} defines the code-formation problem. Section~\ref{sec:framework} gives the formal setup. Section~\ref{sec:theorem} states the projection, non-recoverability, partition, convergence, forced-nontriviality, and anticipation results, with a worked one-bit example. Section~\ref{sec:emergence} shows that the predicted partition is reachable from random initial conditions under selection on coordination loss, and that channel capacity shapes the effective codeword alphabet. Section~\ref{sec:mechanism} sketches biological implementation. Section~\ref{sec:cases} lists biological instances. Sections~\ref{sec:predictions}--\ref{sec:objections} give predictions and address objections. Section~\ref{sec:prior} situates the work in the prior papers. Section~\ref{sec:conclusion} concludes.
...
\paragraph{Does this prove all biological systems form codes?} No, and deliberately so. Proposition~\ref{prop:forced} states sufficient conditions: pointwise $\gamma$-action-incompatibility of $r$ regions of mass at least $p$, finite reliable boundary alphabet, and low-regret viability pressure. Where those conditions are met, codes are forced. Where they are not --- where the substrate's task-relevant action structure is fully integrable across the relevant boundary, where channel capacity already supports continuous coupling, or where viability pressure is weak --- the framework predicts continuous coupling rather than discrete codes (Corollary~\ref{cor:nocode}). The biological cases of Section~\ref{sec:cases} argue that the conditions are typical at the substrate boundaries listed in Table~\ref{tab:cases}, not that they are universal across all biological organization. Establishing the conditions empirically in any specific biological substrate is a separate task from proving the implication.
