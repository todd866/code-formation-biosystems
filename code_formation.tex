...
\caption{The useful one-bit code preserves the future-relevant slow-mode class, not an arbitrary present coordinate. Substrate $X_t=(\cos\phi_t,\sin\phi_t)+\eta_t$ with $\sigma=0.42$ and $n=120{,}000$. (a) Substrate at $\Delta=\pi/2$; colour encodes the future class $F_{t+\Delta}$, the conceptual hinge of the figure. The dashed cut $X_1=0$ labels the present coordinate; the solid cut is the best one-bit partition (the future-relevant cut). (b) Accuracy and mutual information at $\Delta=\pi/2$ for random, present-axis, best one-bit, and oracle codes. (c) Sweep over $\Delta\in[0,\pi]$: the best one-bit accuracy stays high while the present-axis accuracy varies sinusoidally. Inset: angular error of the recovered projection relative to the theoretical optimum, near zero across the sweep (a secondary check that the search recovers the theory). Reproducible code in \texttt{simulations/generate\_figures.py}.}
