...
\begin{abstract}
Living systems are high-dimensional, bounded, self-maintaining processes. Their internal dynamics contain more task-relevant structure than any membrane, synapse, receptor interface, germline transition, or other boundary can transmit in full. This paper treats code formation as the constitutive solution to that boundary problem: recurrent viability-coupled projection converts a high-dimensional substrate into a finite or effectively finite, interpreted, action-guiding partition. The claim is not that every biological variable is a code, nor that every category imposed by an observer is a code. It is that when bounded self-maintaining dynamics must coordinate through finite-throughput interfaces, and when recurrent substrate regimes require incompatible responses, a no-code solution pays a viability cost.

The paper formalizes this boundary-code principle. A rate-distortion and data-processing argument shows why direct state sharing fails and why discarded distinctions cannot be recovered downstream. Retained low-loss protocols therefore take the form of finite partitions of substrate state, with fibers coherent under their assigned responses. Under finite or discretized conditions, an alternating Lloyd--Bayes improvement dynamics on encoder--decoder pairs decreases risk monotonically and has partition-shaped stationary limit points. When the substrate contains mutually action-incompatible regimes, any low-regret protocol must allocate at least one reliable codeword to each such regime; below that capacity, an explicit regret floor acts. Embedded in independently motivated axioms about bounded self-maintenance, this yields a boundary-code necessity result: any system satisfying the axioms must maintain at least one nontrivial code at one of its boundaries or incur a persistence-threatening regret floor. A sufficient-statistic argument extends the principle across timescales: anticipatory content can reside in the boundary message, in the receiver's entrained context, or in both. A multi-layer hourglass simulator then visualizes the predicted boundary flow: high-dimensional bulk dynamics collapse through a finite-alphabet waist, partially re-expand downstream without recovering discarded task information, and preserve the task-relevant slow mode while aliasing irrelevant modes. The framework predicts where codes appear, where continuous coupling persists, and how channel bandwidth, slow-mode coupling, and viability pressure shape the partition. It does not derive any particular alphabet such as the codon table; it explains why finite interpreted alphabets are expected at biological boundaries.
\end{abstract}
...
Living systems are high-dimensional, bounded, self-maintaining processes. A cell contains molecular, metabolic, electrical, mechanical, and regulatory degrees of freedom that no membrane interface can transmit in full. A tissue contains spatial and developmental structure that no local cell-fate signal can carry in full. A nervous system contains distributed oscillatory state that no spike train or axon bundle can carry in full. A lineage contains a physiological trajectory that no gamete or genome-level interface can carry in full. In each case, the system must coordinate across a boundary too narrow for the dynamics themselves.

This paper treats biological codes as the stable solution to that boundary problem. A finite boundary does not merely compress a high-dimensional substrate. When the projection recurs, when downstream systems interpret the projection, and when viability or regulation preferentially retains projections that work, the compression becomes a reusable action-relevant partition. That is the sense in which a code is formed. Nucleotide triplets, spike and phase patterns, morphogen thresholds, MHC--peptide complexes, hormone regimes, and quorum signals differ mechanistically, but they share this boundary structure: many upstream states are collapsed into a smaller set of downstream-interpretable classes because those classes preserve distinctions that matter for action.
...
The claim is intentionally constitutive rather than accidental. Given a high-dimensional bounded substrate, finite boundary throughput, recurrent coordination, action-incompatible regimes, and retention of lower-loss mappings, code formation is not an optional overlay; it is what finite boundaries do to self-maintaining dynamics. This is a circularity only in the benign sense in which a heat engine constitutively involves heat exchange across a temperature gradient. The empirical content is not that ``some partition exists'' in the abstract. It is the more restrictive claim that biological codes should be found where the boundary, the action-incompatible regimes, the receiver, the recurrent partition, and the cost of aliasing can all be identified.

A code is therefore not any category. It is a boundary partition when it is (i) finite or effectively finite at the boundary, (ii) recurrent across episodes, (iii) interpreted by a downstream mechanism, and (iv) action-guiding in a way that improves viability, regulation, or coordination.
...
The rest of the paper is organised as follows. Section~\ref{sec:problem} defines the code-formation problem and separates trivial compression, endogenous code formation, and forced nontrivial codes. Section~\ref{sec:framework} gives the formal setup. Section~\ref{sec:theorem} proves the projection, non-recoverability, cascade, partition, convergence, forced-nontriviality, and anticipation results, with a worked one-bit example. Section~\ref{sec:universal} axiomatises bounded self-maintenance and states the boundary-code necessity result. Section~\ref{sec:emergence} reports simulations as boundary-flow demonstrations, including the volumetric hourglass simulator. Sections~\ref{sec:mechanism}--\ref{sec:cases} sketch biological implementation and selected cases; Sections~\ref{sec:predictions}--\ref{sec:objections} give predictions and address objections; Section~\ref{sec:prior} situates the work in the prior papers; Section~\ref{sec:conclusion} concludes.

\section{The code-formation problem}
\label{sec:problem}

Genetic codons, neural spikes, morphogen labels, immune motifs, hormone regimes, and quorum signals differ mechanistically but share a structure: a high-dimensional substrate produces a finite or low-dimensional signal that crosses a boundary, a downstream system treats the signal as selecting among possible responses, and the mapping recurs across episodes and is stabilized by selection, learning, development, or homeostasis \citep{levin2023,levin2025multiscale}. The code-formation problem is to explain why this structure emerges. It is not transmission (which begins with a source alphabet), nor convention (which presupposes signal-choosing agents), nor representation (which asks what a symbol means): it is the prior question of why a substrate is partitioned into symbol-bearing classes at all.
...
Three claims must be kept separate. \emph{Trivial compression} occurs whenever a finite channel receives input from a richer substrate; by itself this is not a code. \emph{Endogenous code formation} occurs when the compressed message is recurrent, interpreted by a downstream mechanism, and stabilized because it improves coordination or viability. \emph{Forced nontrivial code formation} occurs when the substrate contains recurrent regimes that cannot share a near-optimal response; aliasing them into one message incurs a regret floor, so a low-regret system must allocate distinct reliable messages. The formal results below concern the second and third claims, not arbitrary compression.
...
The boundary can be a membrane, synapse, axon bundle, receptor--ligand interface, endocrine route, developmental commitment point, or germline transition. What matters is that the channel from one dynamical regime to another is capacity-limited relative to the source substrate.
...
\section{Boundary-code necessity}
\label{sec:universal}

Proposition~\ref{prop:forced} is conditional. It says that when recurrent action-incompatible regimes must coordinate through a finite boundary, low-regret protocols require distinct reliable messages. The biological question is whether bounded self-maintaining systems generally instantiate those conditions. The answer below is again conditional and constitutive: if a system satisfies a small set of independently motivated conditions for bounded self-maintenance, then the no-code option carries a persistence-threatening regret floor. The axioms are not intended to smuggle in ``code'' by definition; they specify boundary closure, non-equilibrium maintenance, finite throughput, high-dimensional interior dynamics, action-incompatible regimes, and retention. Codes are what follows when these conditions meet a finite boundary.
...
\begin{proposition}[Boundary-code necessity for bounded self-maintenance]
\label{prop:universal}
Any system satisfying Axioms~\ref{ax:closure}--\ref{ax:retention} must maintain at least one nontrivial boundary code, or else any protocol it maintains across the relevant boundary pays a regret floor incompatible with persistence on the relevant timescale. Equivalently: under these axioms, the no-code solution is not viable.
\end{proposition}
...
The conclusion is the minimal result: at least two codewords. Real biological systems often satisfy the conditions for $r \gg 2$ --- codons, spike alphabets, cell-fate alphabets, immune repertoires --- because the action-incompatibility number of an organized living substrate is large; the proposition only guarantees the floor.

\subsection{Status of the necessity claim}

The axioms do not mention codes, partitions, or messages. They describe boundary closure, non-equilibrium maintenance, finite throughput, multi-component interior, action-incompatible regimes, and retention --- each operationally specifiable without reference to coding. The argument is nevertheless constitutive: once these conditions are granted, code formation is not a surprising add-on but the finite-boundary form of self-maintenance. The result is therefore not ``biology forms codes because we have defined biology as code-forming,'' but ``bounded high-dimensional self-maintenance plus finite throughput and incompatible action regimes leaves only three options: fail, increase capacity, or stabilize an interpreted partition.'' Biological systems persist by the third route wherever the first two are unavailable.

\begin{corollary}[Biological code necessity under the axioms]
\label{cor:calibrated}
Within the axiomatic framework of Axioms~\ref{ax:closure}--\ref{ax:retention}, any system satisfying the axioms must maintain at least one nontrivial code at at least one of its boundaries. Insofar as the axioms accurately characterize living organization, every living system has at least one such boundary code. The statement is conditional on the axioms and does not imply that every biological variable, category, or signal is a code.
\end{corollary}
...
Section~\ref{sec:convergence} gives an idealized alternating-improvement mechanism for which partition-shaped fixed points exist. Real biological selection, learning, and plasticity are not literal Lloyd--Bayes iteration; nor are real biological boundaries sharp two-zone interfaces. The simulations below are therefore not biological validation. They are boundary-flow demonstrations: controlled systems in which one can watch high-dimensional substrate variation approach a finite boundary, collapse into an action-relevant waist, and partially re-expand downstream without recovering distinctions discarded at the waist. The experiments instantiate three signatures predicted by the theory: (i)~selection finds an action-relevant partition rather than an arbitrary classifier; (ii)~the cardinality floor of Proposition~\ref{prop:forced} binds when effective waist capacity falls below the action repertoire; and (iii)~task-relevant slow-mode information survives the waist while task-irrelevant modes are aliased away.
...
The single-channel demonstration above hides the radial structure of real boundaries. To visualize the boundary flow directly, we run a multi-layer hourglass: a continuous high-dimensional bulk substrate, a chain of continuous Linear+$\tanh$ encoder layers progressively narrowing the representation, a single discrete waist with $K$ codewords (Gumbel--Softmax with straight-through estimation \citep{jang2017,maddison2017,vandenoord2017}), a chain of continuous decoder layers re-expanding the representation, and a final $R$-way classifier head. The continuous chain implements radial coarse-graining; the discrete waist enforces the partition. The point is not that biology literally uses this architecture, but that the architecture exposes the quantities the theory says should matter: upstream task information, finite reliable messages, downstream interpretation, and the irreversibility of discarded distinctions.

Throughout this section we report participation-ratio $D_{\rm eff}$ as a descriptive collapse signature, not as a code-capacity metric. Capacity claims are carried by $K_{\rm eff}=2^{H(M_{\rm waist})}$, by the cardinality floor of Proposition~\ref{prop:forced}, and by the mutual-information measurements at the waist; $D_{\rm eff}$ is reported because it visualizes the radial flow predicted by Lemma~\ref{lem:cascade}, not because it bounds symbol capacity, separability, or noise robustness on its own.
...
\paragraph{Is the argument circular?} It is constitutive, not viciously circular. If a system is high-dimensional, bounded, recurrently self-maintaining, and forced to coordinate through finite interfaces, then interpreted partitions are the form that successful boundary coordination takes. The nontrivial work is not proving that some coarse-graining exists; every finite channel coarse-grains. The nontrivial work is specifying when the coarse-graining becomes endogenous, recurrent, action-guiding, and retained; deriving the capacity and regret floor for action-incompatible regimes; and identifying the boundary-flow signatures by which such codes can be measured. The claim can fail whenever one of those conditions fails: no finite boundary, no recurrent action-incompatible regimes, no downstream interpretation, no retention, or enough continuous bandwidth to transmit the relevant dynamics directly.
...
\paragraph{Does this prove all biological systems form codes?} It proves a conditional necessity result. Within the axiomatic framework of Section~\ref{sec:universal}, any system satisfying the axioms must maintain at least one nontrivial boundary code or pay a persistence-threatening regret floor (Corollary~\ref{cor:calibrated}). Outside the framework, the claim becomes empirical: one must identify the finite boundary, the action-incompatible regimes, the downstream interpreter, the recurrent partition, and the cost of aliasing. The contingent structure of any specific code (codon table, particular spike alphabets, cell-fate repertoires) is not derived; nor are systems violating Axiom~\ref{ax:nonequilibrium} (dormant spores under sufficient quiescence) covered until they re-enter active maintenance.
...
In one line: a finite-bandwidth boundary does not merely compress a high-dimensional biological substrate; under viability-coupled recurrence, it converts the substrate into a finite action-sufficient partition, leaving the remaining internal degrees of freedom as within-codeword shadow complexity. This reframes biological codes as consequences of physical constraint rather than optional representational overlays. Genetic codons, neural spikes, circadian gates, morphogenetic fates, immune motifs, hormones, and quorum signals differ mechanistically but solve the same abstract problem: how to coordinate across a boundary too narrow for the dynamics themselves. Where continuous coupling suffices, no code is needed. Where it fails, retained low-loss solutions take code-like form. Biology uses codes because boundaries force compression, and viability teaches the compression what to preserve.
