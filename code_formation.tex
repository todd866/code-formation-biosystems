...
\paragraph{Stochastic encoders.} The encoder $\pi$ is written as a deterministic function for clarity. The analysis carries through for stochastic encoders by replacing the fiber $\pi^{-1}(m)$ with the high-probability decision region $\{x:P(M=m\mid X=x)\geq 1-\rho\}$ for a chosen reliability $\rho$, and reading the coherence condition conditionally on the received message. Channel noise enters through the reliable-codebook bound of Definition 2; the structural results below apply to whichever portion of the substrate the boundary distinguishes reliably.
...
\I(U;Y) \leq \I(U;M).
\]
Distinctions in $U$ that the projection has discarded cannot be recovered downstream.
\end{lemma}
...
\end{enumerate}
Then any retained low-loss protocol takes the form of a finite partition of $\X$ whose occupied fibers, away from a small exception set, are coherent under their assigned responses (Definition~\ref{def:fiber}). If the partition is reused across episodes and interpreted by downstream dynamics, the protocol is a biological code. This is a structural characterization of low-loss retained protocols; an explicit alternating-improvement dynamics whose limit points realize this structure is given in Section~\ref{sec:convergence}.
\end{proposition}
...
Let $\M$ be a finite alphabet, $\A$ a compact action set, $\ell$ a continuous viability loss, and $P_X$ a substrate distribution.
\begin{enumerate}
...
This is the substantive condition. Non-equilibrium maintenance plus a non-trivial life cycle motivates it: replication, repair, defense, and growth phases are not interchangeable in their action requirements, and an organism cycling between them faces regimes whose optimal responses differ. A perfectly homogeneous gradient-free environment over the persistence timescale removes such regimes and triggers the no-code regime of Corollary~\ref{cor:nocode}, but Axiom~\ref{ax:nonequilibrium} excludes that limit for living systems. The theorem requires Axiom~\ref{ax:incompat} explicitly; the connection is motivation, not derivation.
...
\begin{proof}
Axiom~\ref{ax:closure} supplies the boundary; Axiom~\ref{ax:finite} the finite reliable throughput; Axiom~\ref{ax:highd} the high-dimensional interior; Axiom~\ref{ax:incompat} two action-incompatible regimes $S_1, S_2$ of mass $p>0$ with margin $\gamma>0$; Axiom~\ref{ax:nonequilibrium} their recurrence; Axiom~\ref{ax:retention} viability-coupled retention. If expected regret exceeded $\gamma p$ on persistence timescales, the regret-floor clause of Proposition~\ref{prop:forced} would force persistence failure, contradicting Axiom~\ref{ax:nonequilibrium}. Proposition~\ref{prop:forced} then forces the protocol to use at least two distinct reliable codewords across $S_1 \cup S_2$.
\end{proof}
...
Within the axiomatic framework of Axioms~\ref{ax:closure}--\ref{ax:retention}, every living system forms at least one nontrivial code at at least one of its boundaries.
\end{corollary}
...
A bulk-channel ablation (decoder receives codeword, low-dimensional bulk context, both, or both with bulk shuffled at evaluation) and a stochastic-resonance demonstration with coupled Kuramoto networks support the latent-context route of Proposition~\ref{prop:latent}: when bulk--environment synchronization is destroyed at evaluation while codeword identity is preserved, the synchronization-dependent component of viability collapses, while a code-only architecture is unaffected. Reproducible code is in \texttt{simulations/code\_vs\_bulk\_ablation.py} and \texttt{simulations/stochastic\_resonance\_sweep.py}; full results in the corresponding \texttt{.txt} files.
