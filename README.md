...
Biology is full of codes: codons, neural spikes, receptor-ligand interfaces, morphogenetic signals, immune motifs, hormones, and quorum signals. Existing theories describe what such codes do; this paper develops a constraint-based account of why they form. The central setup is a *dimensional boundary*: an interface where the relevant state space on one side has more task-relevant degrees of freedom than the channel can transmit. Under recurrent viability pressure, finite-boundary projection becomes an endogenous, interpreted, action-guiding partition. When the substrate contains mutually action-incompatible regimes, those partitions are *forced* to allocate distinct codewords to them. Codes are not merely admitted; under the stated boundary conditions, the no-code option pays a viability cost.
...
- **Boundary-code necessity (§5, Cor 7)**: Six axioms of bounded self-maintenance — autopoietic closure, non-equilibrium maintenance, finite throughput, high-dimensional interior, action-incompatible regimes, retention — jointly imply the forced-code conditions. Any system satisfying the axioms must maintain at least one nontrivial boundary code or pay a persistence-threatening regret floor. This is a conditional/constitutive claim, not the claim that every biological category is a code.
